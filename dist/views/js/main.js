...
/* replace querySelectorAll("#randomPizzas") with document.getElementsByClassById("randomPizzas")
   and put it in a variable for reuse */
...
var resizePizzas = function(size) {
  ...
  /* Refactor determineDX function to minimize browser reflow
  Change the calculation of newwidth using "WindowWidth * size ratio" to
  minimize usage of offsetWidth */

  var windowWidth = pizzaContainer.offsetWidth;
  function caculateNewWidth (size) {

    var newwidth;

      switch(size) {
        case "1":
          newwidth = windowWidth * 0.25;
          break;
        case "2":
          newwidth = windowWidth * 0.3333;
          break;
        case "3":
  ...
  /* replace querySelectorAll(".randomPizzaContainer") with document.getElementsByClassName("randomPizzaContainer")
     and put it in a variable for reuse */

  var randomPizzaContainers = document.getElementsByClassName("randomPizzaContainer");
  var randomPizzaLen = randomPizzaContainers.length;
  function changePizzaSizes(size) {
    var newwidth = caculateNewWidth(size) + "px";
    for (var i = randomPizzaLen; i--;) {
...
/* append pizza element to DocumentFragment and finish by append it to the Dom */
var pizzaFragment = document.createDocumentFragment();
for (var i = 2; i < 100; i++) {
  pizzaFragment.appendChild(pizzaElementGenerator(i));
}
pizzaContainer.appendChild(pizzaFragment);
...
function updatePositions() {
  ...
  /* replace querySelectorAll('.mover') with getElementsByClassName("mover")
     and put it in a variable for reuse */
  var items = document.getElementsByClassName("mover");

  /* Precaculate the five dx values and put them in phases array in order to move
     calculation out of For loop */
  var phases = [];
  ...
  for (var i = 0, itemLen = items.length; i < itemLen; i++) {
    ...
    /* Replace style.left with transform to minize repaints and reflows*/
    /* correct the sliding issue */
    var dx = 100 * phases[i % 5];
    items[i].style.transform = "translateX(" + dx + "px)";
  }
...
/* Using requestAnimationFrame on scroll event */
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var numPizzas = Math.ceil(windowHeight / s) * cols;

  var movingPizzas = document.getElementById("movingPizzas1");
  var fragment = document.createDocumentFragment();
  /* Cut down amount of pizzas from 200 to 36 */
  for (var i = numPizzas; i--;) {
    ...
    /* indicate element's start position */
